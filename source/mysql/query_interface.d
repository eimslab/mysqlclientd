...
class QueryInterface {
    ...
    static string makeQuery(Connection db, string sql, ...) {
        Variant[] args;
    ...
    */
    ...
    static string toSql(Variant a, Connection db) {
        auto v = a.peek!(void*);
        if(v && (*v is null))
            return "NULL";
        else {
    ...
    static string toSql(string s, Connection db) {
        if(s is null) return "NULL";
        return '\'' ~ db.escape(s) ~ '\'';
    }

    static string toSql(long s, Connection db) {
        return to!string(s);
    }

    static string toSqlName(string s, Connection db) {
        if(s is null) return "NULL";
        return db.escape(s);
    }

    static string toSqlName(long s, Connection db) {
        return toSql(s, db);
    }

    static string toSqlName(Variant a, Connection db) {
        auto v = a.peek!(void*);
        if(v && (*v is null))
            return "NULL";
        else {
    ...
    static string toSqlArray(long s, Connection db) {
        return toSql(s, db);
    }

    static string toSqlArray(Variant a, Connection db) {
        auto v = a.peek!(void*);
        if(v && (*v is null))
            return "NULL";
        else {
    ...
    static string escapedVariants(Connection db, in string sql, Variant[string] t) {
    ...
    static string escapedVariants(Connection db, in string sql, Variant[] t) {
        // FIXME: let's make ?? render as ? so we have some escaping capability
